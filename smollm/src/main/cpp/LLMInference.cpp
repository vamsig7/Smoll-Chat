...
LLMInference::completionLoop() {
    ...
    if (_isValidUtf8(_cacheResponseTokens.c_str())) {
        ...
        // check for stop words in the accumulated response
        if (!_stopWords.empty()) {
            for (const auto & sw : _stopWords) {
                if (!sw.empty()) {
                    size_t pos = _response.find(sw);
                    if (pos != std::string::npos) {
                        // trim response up to the stop word and finish generation
                        _response.resize(pos);
...
void
LLMInference::setStopWords(const std::vector<std::string>& stopWords) {
